...
namespace CommunicationWithSTM32
{
	internal class HIDDevice
	{
		...
		private void initDevice(string devicePath, bool useAsyncReads)
		{
			...
			//create file handles using CT_CreateFile
			handle_read = CreateFile(devicePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);
			handle_write = CreateFile(devicePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);
		...
		{
			if (data.Length > capabilities.OutputReportByteLength) throw new Exception("Output report must not exceed " + (capabilities.OutputReportByteLength - 1).ToString() + " bytes");
			...
			if (FS_write.CanWrite) FS_write.Write(packet, 0, packet.Length);
			else
